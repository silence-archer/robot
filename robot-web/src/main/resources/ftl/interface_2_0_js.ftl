...
form.val('testSys', response.data.data.SYS_HEAD);
<#list arrays as array>
...
const sysHead = form.val("testSys");
sysHead["SERVICE_CODE"] = "${name}".split("-")[0];
sysHead["MESSAGE_TYPE"] = "${name}".split("-")[1];
sysHead["MESSAGE_CODE"] = "${name}".split("-")[2];
<#list arrays as array>
body["${array.name}"] = table.getData("test${array?index}");
</#list>
$http.post(baseUrl+"loan",{
apiCd: $('#ipOwner').val()+":"+$('#port').val()+"${uri}",
data: {'BODY':body,'SYS_HEAD':sysHead}
